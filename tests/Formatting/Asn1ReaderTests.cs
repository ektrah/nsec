...
namespace NSec.Tests.Formatting
{
    public static class Asn1ReaderTests
    {
        ...
        {
            var span = new ReadOnlySpan<byte>(new byte[] { 0x05, 0x00 });
            var reader = new Asn1Reader(ref span);
            reader.Null();
            Assert.True(reader.SuccessComplete);
        }
        ...
        public static void SequenceStackOverflow()
        {
            ...
            var reader = new Asn1Reader(ref span, 3);
        ...
        {
            var span = new ReadOnlySpan<byte>(value);
            var reader = new Asn1Reader(ref span, depth + 1);
        ...
        {
            var span = new ReadOnlySpan<byte>(new byte[] { 0x30, 0x03, 0x02, 0x01, 0x17, 0x02, 0x01, 0x42 });
            var reader = new Asn1Reader(ref span);
            reader.BeginSequence();
            Assert.True(reader.Success);
